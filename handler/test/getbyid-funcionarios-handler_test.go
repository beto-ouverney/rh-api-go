...
func TestGetById(t *testing.T) {
	...
	tests := []struct {
		...
		{
			describe:        "Deve ser capaz de retornar os dados de uma funcionario de acordo com o id",
			id:              "1",
			expectedStatus:  200,
			expectedMessage: allFuncMock[0],
			msg:             "Stattus code deve ser igual",
			msg1:            "Deve ser capaz de retornar os dados de uma funcionario de acordo com o id",
		},
	}
	for _, tt := range tests {
		t.Run(tt.describe, func(t *testing.T) {
			path := fmt.Sprintf("/funcionarios/%s", url.QueryEscape(tt.id))
			req := httptest.NewRequest(http.MethodGet, path, nil)
			rr := httptest.NewRecorder()
			router.ServeHTTP(rr, req)

			assertions.Equal(tt.expectedStatus, rr.Code, tt.msg)
			var actual entity.Funcionario
